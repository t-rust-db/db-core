//! Journal-mode SHARED byte-range locking via `sql_sys::fcntl` (a
...
use sql_sys::fcntl::{fcntl_call, off_t, FcntlArg, F_RDLCK, F_UNLCK, F_WRLCK};
...
fn check_reserved_lock(file: &File) -> io::Result<bool> {
    let mut fl = sql_sys::fcntl::flock {
        l_type: F_WRLCK,
        l_whence: SEEK_SET,
...
mod tests {
    ...
    use crate::test_lock_probe::lock_held_by_subprocess;
    ...
    fn lock_shared_fails_with_contention_errno_when_exclusively_held_elsewhere() {
        use crate::{UnixVfs, Vfs, VfsError};
    ...
    fn reserved_lock_available(path: &std::path::Path) -> bool {
        crate::test_lock_probe::lock_available(path, "wrlock", RESERVED_BYTE, 1)
    }

    fn pending_lock_available(path: &std::path::Path) -> bool {
        crate::test_lock_probe::lock_available(path, "wrlock", PENDING_BYTE, 1)
    }

    fn shared_read_available(path: &std::path::Path) -> bool {
        crate::test_lock_probe::lock_available(path, "rdlock", SHARED_FIRST, SHARED_SIZE)
    }
