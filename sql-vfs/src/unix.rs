...
use sql_sys::fcntl::{EACCES, EAGAIN};
...
impl Vfs for UnixVfs {
    ...
    fn open_wal_shm(&self, path: &Path) -> Result<Option<crate::AnyWalShm>> {
        let shm_path = companion_path(path, "-shm");
        if !shm_path.exists() {
            return Ok(None);
        }
        shm::open_wal_shm(&shm_path)
            .map(|handle| {
                Some(crate::AnyWalShm::from(
                    Box::new(handle) as Box<dyn crate::WalShm>
                ))
            })
            .map_err(|source| to_vfs_error(&shm_path, source))
    }
...
impl VfsFile for UnixVfsFile {
    ...
    // alone calls plain `fsync()` there. `sql_sys::fcntl::fsync`
    ...
    #[cfg(target_os = "macos")]
    fn sync(&self) -> Result<()> {
        sql_sys::fcntl::fsync(self.lock.borrow().file())
            .map_err(|source| to_vfs_error(&self.path, source))
    }
