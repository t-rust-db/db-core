...
- **`src/codegen/batch.rs` is now `src/codegen/batch_plan.rs`** (module path `codegen::batch` unchanged via `#[path]`), so its MC/DC obligation ids (`<file-stem>_<line>`) can no longer collide with `src/vm/batch.rs`'s; the line-padding comments both files carried to dodge that are gone.
