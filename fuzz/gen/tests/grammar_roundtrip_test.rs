...
fn sqlite_section_carries_v_block_tags() {
    ...
    assert!(has_tags, "sqlite-rs section is documented to carry (* Vn *) tags");
}
...
fn column_and_shared_sections_reference_only_known_rules_or_primitives() {
    ...
    let primitives = ["ALPHA", "DIGIT", "IDENT", "INTEGER", "any", "character", "except"];
    for rule in g.column.values().chain(g.shared.values()) {
        for seq in &rule.alternatives {
            for name in nonterminal_names(seq) {
                let known = g.column.contains_key(&name) || g.shared.contains_key(&name) || primitives.contains(&name.as_str());
                assert!(known, "rule '{}' references unknown nonterminal '{}'", rule.name, name);
            }
        }
    }
}
...
fn walker_generates_bounded_output_for_each_entry_rule() {
    let g = grammar();
    let entries = [
        (Section::Column, "sql-stmt"),
        (Section::Sqlite, "sql-stmt"),
    ];
    for (section, entry) in entries {
        ...
        assert!(exercised > 0, "walker must have exercised at least one alternative");
        assert!(in_scope > 0, "grammar must have in-scope alternatives");
        assert!(exercised <= in_scope, "exercised count cannot exceed in-scope count");
    }
}
...
fn walker_is_seed_stable() {
    ...
    for _ in 0..10 {
        assert_eq!(a.generate("sql-stmt").expect("walk ok"), b.generate("sql-stmt").expect("walk ok"));
    }
}
...
fn stream_expr_entry_walks_with_landed_vblock_scope() {
    let g = grammar();
    let landed: std::collections::HashSet<String> = ["V1", "V2", "V3"].iter().map(|s| s.to_string()).collect();
