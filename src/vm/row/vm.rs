...
pub enum ExecError {
    ...
    /// A record blob in a register (`OpenPseudo`'s source register, or the
    /// blob handed to `OpenPseudo`) did not decode. Record corruption used
    /// to read as a NULL column; it is a typed error instead (db-core#232).
    RecordDecode {
        /// Name of the opcode that read the record.
        opcode: &'static str,
        /// What the record decoder rejected.
        source: super::record::RecordError,
    },
...
impl std::fmt::Display for ExecError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ExecError::RecordDecode { opcode, source } => {
                write!(f, "{opcode}: record blob does not decode: {source}")
            }
...
impl Vm {
    ...
    fn is_null_row(&self, slot: i32) -> Result<bool, ExecError> {
        ...
        // The flag table grows lazily; a slot never written is not NULL-rowed.
    ...
    fn next_sequence(&mut self, slot: i32) -> Result<i64, ExecError> {
        ...
        let base = self.cursor(slot)?.sequence_base();
    ...
    /// Host parameter `index` (1-based, as in `?NNN`). `Ok(None)` is an
    /// unbound parameter -- SQLite reads those as NULL; a zero or negative
    /// index is a malformed instruction, not "unbound" (db-core#232).
    fn param(&self, index: i32) -> Result<Option<&Value>, ExecError> {
        let idx = usize::try_from(index)
            .ok()
            .and_then(|i| i.checked_sub(1))
            .ok_or_else(|| ExecError::MalformedInstruction {
                opcode: "Variable",
                reason: format!("parameter index {index} is not >= 1"),
            })?;
        Ok(self.params.get(idx))
    }
    ...
    pub(crate) fn bounded_count(opcode: &'static str, count: i32) -> Result<usize, ExecError> {
        match usize::try_from(count) {
            Ok(n) if n <= MAX_REGISTERS => Ok(n),
            _ => Err(ExecError::RegisterRangeTooLarge { opcode, count }),
        }
    }
    ...
    /// The register a pseudo-cursor slot is bound to, `None` for any
    /// other slot (including a negative one -- the `cursor(slot)?` that
    /// follows every `pseudo_reg` miss raises `CursorNotOpen` for those).
    ...
    /// Hands the collected result rows to the caller and leaves the VM's
    /// buffer empty, without copying -- what [`execute`] returns at
    /// `Halt` (#254). A `Vm` driven through `execute` more than once
    /// therefore starts each run with no rows carried over.
    pub fn take_rows(&mut self) -> Vec<Vec<Value>> {
        std::mem::take(&mut self.rows)
    }

    /// Grows the register file once to cover registers `0..=upto`, so
    /// the per-write `resize` in [`Self::set_register`] never fires
    /// inside the dispatch loop (#254). A request past [`MAX_REGISTERS`]
    /// is ignored here; the first out-of-range write still reports it.
    pub fn reserve_registers(&mut self, upto: i32) {
        let Some(len) = usize::try_from(upto).ok().map(|n| n.saturating_add(1)) else {
            return;
        };
        if len <= MAX_REGISTERS && len > self.registers.len() {
            self.registers.resize(len, Value::Null);
        }
    }
}
...
fn step(vm: &mut Vm, pc: usize, instr: &Instruction) -> Result<Step, ExecError> {
    match instr.opcode {
        ...
        Opcode::Variable => {
            // An unbound host parameter reads as NULL (SQLite semantics).
        ...
        Opcode::MakeRecord => {
            ...
            let mut values = Vec::with_capacity(count);
            for i in 0..count {
                let reg = i32::try_from(i)
                    .ok()
                    .and_then(|i| instr.p1.checked_add(i))
                    .ok_or(ExecError::RegisterOutOfRange {
                        opcode: "MakeRecord",
                        index: instr.p1,
                    })?;
        ...
        Opcode::OpenPseudo => {
            ...
            // Placeholder only: the row is read live from register `p2`
            // (`pseudo_column`), never from this cursor's own values.
            vm.open_cursor(instr.p1, Box::new(PseudoCursor::default()))?;
        ...
        Opcode::SorterOpen => {
            ...
            let bound = if instr.p5 == 0 {
                ...
                // `LIMIT -1` (any negative) means "no limit" in SQLite, and a
                // non-integer limit register is unbounded too.
        ...
        Opcode::SorterInsert => {
            ...
            if !inserted {
                return Err(ExecError::MalformedInstruction {
                    opcode: "SorterInsert",
                    reason: "cursor slot is not a sorter, or the record blob does not decode"
                        .to_string(),
                });
            }
            Ok(Step::Next)
        ...
        Opcode::HashAggFind => {
            ...
            if !found {
                return Err(ExecError::MalformedInstruction {
                    opcode: "HashAggFind",
                    reason:
                        "cursor slot is not a hash-aggregation cursor, or the record blob does not decode"
                            .to_string(),
                });
            }
            Ok(Step::Next)
        ...
        Opcode::HashAggData => {
            ...
            for (slot, state) in accumulators.into_iter().enumerate() {
                // Saturation is unreachable (slots are bounded by
                // MAX_REGISTERS) and would be caught by set_agg_context's
                // own range check anyway.
...
pub fn execute(vm: &mut Vm, program: &Program) -> Result<Vec<Vec<Value>>, ExecError> {
    ...
    loop {
        ...
        match step(vm, pc, instr)? {
            ...
            Step::Halt { code: 0, .. } => return Ok(vm.take_rows()),
...
mod tests {
    ...
    fn sequence_hands_out_increasing_values_per_slot() {
        // `Sequence` needs an open cursor on `p1` (codegen always opens the
        // ephemeral table first; an unopened slot is `CursorNotOpen`,
        // db-core#232).
        let mut vm = Vm::new();
        let program = Program::new(vec![
    ...
    /// MC/DC vector (obligation `vm_491`, `Vm::index`'s decision `reg <
    /// 0 || reg as usize > MAX_REGISTERS`): leaf A (`reg < 0`) true.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_index_7f3b3636__v1_negative_register_is_out_of_range() {
        let vm = Vm::new();
        assert!(matches!(
            vm.register(-1),
    ...
    /// `mcdc__vm_row_vm_index_7f3b3636__v1_negative_register_is_out_of_range`.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_index_7f3b3636__v2_in_range_register_is_ok() {
        let vm = Vm::new();
    ...
    /// against `mcdc__vm_row_vm_index_7f3b3636__v2_in_range_register_is_ok`.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_index_7f3b3636__v3_register_past_the_cap_is_out_of_range() {
        let vm = Vm::new();
    ...
    /// MC/DC vector (obligation `vm_670`, `compare_jump`'s decision
    /// `matches!(a, Value::Null) || matches!(b, Value::Null)`): leaf A
    /// (`a` is NULL) true -- no jump is taken regardless of `b`.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_compare_jump_0e991c46__v1_lhs_null_suppresses_the_jump() {
        let rows = run(vec![
            Instruction::new(Opcode::Null, 0, 0, 0),
            Instruction::new(Opcode::Integer, 5, 1, 0),
            Instruction::new(Opcode::Eq, 0, 4, 1),
    ...
    /// `mcdc__vm_row_vm_compare_jump_0e991c46__v1_lhs_null_suppresses_the_jump`.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_compare_jump_0e991c46__v2_neither_null_lets_the_comparison_decide() {
        let rows = run(vec![
            Instruction::new(Opcode::Integer, 5, 0, 0),
            Instruction::new(Opcode::Integer, 5, 1, 0),
            Instruction::new(Opcode::Eq, 0, 4, 1),
    ...
    /// `mcdc__vm_row_vm_compare_jump_0e991c46__v2_neither_null_lets_the_comparison_decide`.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_compare_jump_0e991c46__v3_rhs_null_suppresses_the_jump() {
        let rows = run(vec![
            Instruction::new(Opcode::Integer, 5, 0, 0),
    ...
    /// MC/DC vector (obligation `vm_719`, `binary_op`'s decision
    /// `matches!(a, Value::Null) || matches!(b, Value::Null)`): leaf A
    /// true -- the result is NULL regardless of `b`.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_binary_op_0e991c46__v1_lhs_null_forces_null_result() {
        let rows = run(vec![
            Instruction::new(Opcode::Null, 0, 0, 0),
            Instruction::new(Opcode::Integer, 5, 1, 0),
            Instruction::new(Opcode::Add, 0, 1, 2),
    ...
    /// against `mcdc__vm_row_vm_binary_op_0e991c46__v1_lhs_null_forces_null_result`.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_binary_op_0e991c46__v2_neither_null_runs_the_operation() {
        let rows = run(vec![
            Instruction::new(Opcode::Integer, 2, 0, 0),
    ...
    /// `mcdc__vm_row_vm_binary_op_0e991c46__v2_neither_null_runs_the_operation`.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_binary_op_0e991c46__v3_rhs_null_forces_null_result() {
        let rows = run(vec![
            Instruction::new(Opcode::Integer, 5, 0, 0),
            Instruction::new(Opcode::Null, 0, 1, 0),
            Instruction::new(Opcode::Add, 0, 1, 2),
    ...
    /// MC/DC vector (obligation `vm_739`, `binary_op_reversed`'s
    /// decision `matches!(a, Value::Null) || matches!(b, Value::Null)`):
    /// leaf A (`p1`'s operand) true -- the result is NULL.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_binary_op_reversed_0e991c46__v1_lhs_null_forces_null_result() {
        let rows = run(vec![
            Instruction::new(Opcode::Null, 0, 0, 0),
            Instruction::new(Opcode::Integer, 10, 1, 0),
            Instruction::new(Opcode::Subtract, 0, 1, 2),
    ...
    /// for A against `mcdc__vm_row_vm_binary_op_reversed_0e991c46__v1_lhs_null_forces_null_result`.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_binary_op_reversed_0e991c46__v2_neither_null_runs_the_operation() {
        let rows = run(vec![
    ...
    /// against `mcdc__vm_row_vm_binary_op_reversed_0e991c46__v2_neither_null_runs_the_operation`.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_binary_op_reversed_0e991c46__v3_rhs_null_forces_null_result() {
        let rows = run(vec![
            Instruction::new(Opcode::Integer, 3, 0, 0),
            Instruction::new(Opcode::Null, 0, 1, 0),
            Instruction::new(Opcode::Subtract, 0, 1, 2),
    ...
    /// MC/DC vector (obligation `vm_871`, `try_to_integer`'s decision
    /// `r.fract() == 0.0 && r.is_finite() && in_i64_range(*r)`): baseline
    /// all three leaves true -- a whole, finite, in-range REAL converts.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_try_to_integer_260fb969__v1_whole_finite_in_range_converts() {
    ...
    /// A against `mcdc__vm_row_vm_try_to_integer_260fb969__v1_whole_finite_in_range_converts`.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_try_to_integer_260fb969__v2_fractional_real_does_not_convert() {
        assert_eq!(try_to_integer(&Value::Real(5.5)), None);
    }

    /// MC/DC vector (obligation `vm_871`): leaf B (`is_finite()`) false
    ...
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_try_to_integer_260fb969__v3_infinite_real_does_not_convert() {
    ...
    /// `mcdc__vm_row_vm_try_to_integer_260fb969__v1_whole_finite_in_range_converts`.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_try_to_integer_260fb969__v4_out_of_range_whole_real_does_not_convert() {
    ...
    // #254 tagged MC/DC vectors (obligation `vm_709`):
    // `reserve_registers`'s `len <= MAX_REGISTERS && len > registers.len()`.

    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_reserve_registers_b7e776e8__v1_reserving_past_the_current_file_grows_it() {
        let mut vm = Vm::new();
    ...
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_reserve_registers_b7e776e8__v2_reserving_past_max_registers_is_ignored() {
        let mut vm = Vm::new();
    ...
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__vm_row_vm_reserve_registers_b7e776e8__v3_reserving_within_the_current_file_leaves_it_alone(
    ) {
        let mut vm = Vm::new();
        vm.reserve_registers(7);
