//! `vm::row`'s value model -- ported from sqlite-rs's
//! `record::{value.rs, collation.rs}` (ADR 0008). Deliberately not
//! reused from [`crate::vm::batch::Value`]: the two `Opcode` sets are
//! separate types with separate value models (ADR 0001, ADR 0007), and
//! `batch::Value`'s `Cow<'static, str>` shape exists for AOT-emitted
//! `const` literals, a concern `row` doesn't have.
