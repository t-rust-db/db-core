...
use crate::codegen::row::{Emitter, RegAlloc, Scope};
...
/// [`crate::codegen::row::expr::compile_value`]'s `Subquery` dispatch then
...
pub(crate) fn compile_memoized_scalar_subquery(
    ...
    let col_idx = crate::codegen::row::expr::column_index(&binding.schema, &memo.probe_column)
        .ok_or_else(|| CodegenError::Unsupported {
            reason: format!(
