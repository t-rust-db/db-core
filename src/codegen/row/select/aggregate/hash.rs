...
/// `crate::vm::row::hash_agg`'s module doc for why they are ordered at all.
...
pub(in crate::codegen::row::select) fn try_compile_hash_grouped_scan<F>(
    em: &mut Emitter,
    reg: &mut RegAlloc,
    select: &Select,
    ...
    F: FnMut(&mut Emitter, &mut RegAlloc, i32, i32) -> Result<(), CodegenError>,
{
    ...
    for (expr, target) in select.group_by.iter().zip(&group_targets) {
        ...
        // puts on its group-boundary `Eq` — see `crate::vm::row::hash_agg`
        ...
        group_keys.push(crate::vm::row::GroupKeyColumn {
            index,
            collation,
...
/// descriptor to the same `crate::vm::row::aggregate::step` kernel. The
