...
pub(in crate::codegen::row) enum JoinAccess {
...
/// preference for it.
pub(in crate::codegen::row) fn choose_join_access(
    binding: &TableBinding,
    on_expr: &Expr,
...
pub(in crate::codegen::row) struct AutoIndexProbe {
    ...
    pub(in crate::codegen::row) probe: Expr,
    ...
    pub(in crate::codegen::row) key_column: usize,
}
...
/// [`crate::codegen::row::planner::is_automatic_index_worthwhile`] judging `binding`'s
...
pub(in crate::codegen::row) fn choose_auto_index_probe(
    binding: &TableBinding,
    on_expr: &Expr,
    prior_bindings: &[TableBinding],
) -> Option<AutoIndexProbe> {
    if !crate::codegen::row::planner::is_automatic_index_worthwhile(&binding.stats) {
        return None;
    }
    let (lhs, rhs) = top_level_equality_operands(on_expr)?;
    let (this_side, other_side) = if matches!(&lhs.kind, ExprKind::Column { table, name, .. } if column_belongs_to_binding(binding, table.as_deref(), name))
