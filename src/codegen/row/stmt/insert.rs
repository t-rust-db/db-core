...
use crate::codegen::row::{CondTargets, Emitter, Label, NullTarget, RegAlloc, Target};
...
use crate::parser::row::parse_create_table;
...
fn compile_column_source(
    ...
    match source {
        ColumnSource::Expr(expr) => compile_value(
            em,
            reg,
            &crate::codegen::row::Scope::single(schema, cursor),
            expr,
        ),
...
fn compile_row(
    ...
    for (idx, plan) in plans.iter().enumerate() {
        if Some(idx) == rowid_alias {
            ...
            // `crate::codegen::row::expr::emit_column_read`).
            let r = reg.alloc();
...
fn emit_unique_check(
    ...
    check_schema: &TableSchema,
    index_cursor: i32,
    index: &crate::codegen::row::IndexSchema,
    action: ConflictAction,
    row_skip: Label,
) -> Result<(), CodegenError> {
    let mut start = None;
    ...
    match action {
        ...
        ConflictAction::Replace => {
            ...
            crate::codegen::row::index_maintenance::emit_index_key_ops(
                em,
                reg,
                schema,
                TABLE_CURSOR,
