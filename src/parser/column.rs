...
mod tests {
    ...
    /// MC/DC vector (obligation `column_804`, the CROSS JOIN LIMIT rule
    /// `has_cross_join && select.limit.is_none()`): both leaves true --
    /// the query is rejected.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__column_804__v1_cross_join_without_limit_is_rejected() {
        let err = parse("SELECT id FROM a CROSS JOIN b").unwrap_err();
        assert!(matches!(err, ParseError::Unexpected { .. }));
    }

    /// MC/DC vector (obligation `column_804`): leaf B (`limit.is_none()`)
    /// false with a CROSS JOIN present -- accepted.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__column_804__v2_cross_join_with_limit_is_accepted() {
        let q = parse("SELECT id FROM a CROSS JOIN b LIMIT 10").unwrap();
        assert!(matches!(
            q.limit.as_ref().unwrap().limit.kind,
            ExprKind::Literal(AstLiteral::Integer(10))
    ...
    /// MC/DC vector (obligation `column_804`): leaf A (`has_cross_join`)
    /// false with no LIMIT -- accepted.
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__column_804__v3_non_cross_join_without_limit_is_accepted() {
    ...
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__column_842__v1_agg_without_window_validates_group_by_keys() {
    ...
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__column_842__v2_no_agg_skips_group_by_key_validation() {
    ...
    #[test]
    #[allow(non_snake_case)]
    fn mcdc__column_842__v3_agg_with_window_skips_group_by_key_validation() {
        let q =
    ...
    // db-core#219 tagged MC/DC vectors for the two #198-era decisions in
    // `validate_select` (obligations `column_700`, `column_706`); the
    // snapshot had not been regenerated since #198.

    #[test]
    #[allow(non_snake_case)]
    fn mcdc__column_794__v1_window_beside_computed_expr_is_rejected() {
        assert!(parse("SELECT id + 1, ROW_NUMBER() OVER (ORDER BY id) FROM t").is_err());
    }

    #[test]
    #[allow(non_snake_case)]
    fn mcdc__column_794__v2_window_beside_bare_column_is_accepted() {
        assert!(parse("SELECT id, ROW_NUMBER() OVER (ORDER BY id) FROM t").is_ok());
    }

    #[test]
    #[allow(non_snake_case)]
    fn mcdc__column_794__v3_computed_expr_without_window_is_accepted() {
    ...
    // `column_842` (`has_agg && !has_window`), not a standalone `&&
