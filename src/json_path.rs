//! Minimal, zero-copy JSON value parser plus a JSONPath-lite lookup.
//! Feature-free, dependency-free (ADR 0011): shared by `storage::stream`
//! (ingest-time JSON Lines parsing, #319) and `functions::json_extract`
//! (late parse, #307) so there is exactly one JSON parser in this crate.
//!
//! Only what those two callers need: object/array/string/number/bool/
//! null, borrowing string spans directly from the source (no unescaping
//! -- an escaped string is stored verbatim, backslashes and all;
//! [`json_extract`]'s caller unescapes on demand if it ever needs to).

/// A parsed JSON value, borrowing from the source text.
...
/// JSONPath-lite lookup: `$.a.b` walks nested objects by key; `$` alone
/// (or an empty path) returns the root. No array indexing, no wildcards
/// -- log-line JSON extraction needs neither, and adding either is a
/// separate, unscoped decision (#307 only asks for dotted object paths).
#[must_use]
pub fn lookup<'a>(root: &JsonValue<'a>, path: &str) -> Option<JsonValue<'a>> {
...
mod tests {
    ...
    fn lookup_finds_top_level_key() {
        let (v, _) = parse_value(r#"{"status":"ok","code":200}"#).expect("parses");
        assert_eq!(lookup(&v, "$.status"), Some(JsonValue::Str("ok")));
        assert_eq!(lookup(&v, "$.code"), Some(JsonValue::Int(200)));
    }

    #[test]
    fn lookup_walks_nested_objects() {
        let (v, _) = parse_value(r#"{"req":{"method":"GET"}}"#).expect("parses");
        assert_eq!(lookup(&v, "$.req.method"), Some(JsonValue::Str("GET")));
    }

    #[test]
    fn lookup_returns_none_for_missing_key_or_non_object_path() {
        let (v, _) = parse_value(r#"{"status":"ok"}"#).expect("parses");
        assert_eq!(lookup(&v, "$.missing"), None);
        assert_eq!(lookup(&v, "$.status.nested"), None);
    }

    #[test]
    fn lookup_bare_dollar_returns_root() {
