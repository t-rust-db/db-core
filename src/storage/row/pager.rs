...
//! Lock contention on either surfaces as [`crate::storage::row::vfs::VfsError::Locked`].
...
use crate::storage::row::header::{JournalMode, SynchronousMode};
use crate::storage::row::vfs::{
...
pub struct Pager {
    ...
    tx_lock_level: crate::storage::row::vfs::LockLevel,
    ...
    /// see [`crate::storage::row::vfs::Vfs::open_wal_shm`]'s doc comment. Lazily opened
...
/// Byte offsets of the three header fields ([`crate::storage::row::header::DatabaseHeader`])
...
impl Pager {
    ...
    pub fn open<V: Vfs + Clone + 'static>(
        ...
        Ok(Pager {
            lock,
            tx_lock_level: crate::storage::row::vfs::LockLevel::Shared,
            wal_lock,
            wal_shm: None,
            wal_resume: None,
            source,
    ...
    pub fn flush(&mut self) -> Result<(), PagerError> {
        ...
        self.lock
            .set_level(crate::storage::row::vfs::LockLevel::Exclusive)?;
        ...
        self.lock
            .set_level(crate::storage::row::vfs::LockLevel::Shared)?;
        self.tx_lock_level = crate::storage::row::vfs::LockLevel::Shared;
        result
    }
    ...
    /// with [`VfsError::Locked`](crate::storage::row::vfs::VfsError::Locked) if another
    /// connection already holds RESERVED or higher.
    pub fn begin_immediate(&mut self) -> Result<(), PagerError> {
        self.lock
            .set_level(crate::storage::row::vfs::LockLevel::Reserved)?;
        self.tx_lock_level = crate::storage::row::vfs::LockLevel::Reserved;
        Ok(())
    }
    ...
    pub fn begin_exclusive(&mut self) -> Result<(), PagerError> {
        self.lock
            .set_level(crate::storage::row::vfs::LockLevel::Exclusive)?;
        self.tx_lock_level = crate::storage::row::vfs::LockLevel::Exclusive;
        Ok(())
    }
    ...
    /// `#[cfg(test)]` (#13).
    pub fn tx_lock_level(&self) -> crate::storage::row::vfs::LockLevel {
        self.tx_lock_level
    }
    ...
    fn release_tx_lock(&mut self) -> Result<(), PagerError> {
        if self.tx_lock_level > crate::storage::row::vfs::LockLevel::Shared {
            self.lock
                .set_level(crate::storage::row::vfs::LockLevel::Shared)?;
            self.tx_lock_level = crate::storage::row::vfs::LockLevel::Shared;
        }
    ...
    ///
    /// Claims [`crate::storage::row::vfs::Vfs::claim_wal_write_lock`] for the duration,
    /// so a second concurrent writer is refused (surfaces as
    /// [`crate::storage::row::vfs::VfsError::Locked`], converted to [`PagerError::Vfs`]
    ...
    fn recreate_wal_locked(&mut self) -> Result<wal::WalWriter, PagerError> {
        ...
        shm_file.write_at(&crate::storage::row::vfs::shm::fresh_shm_bytes(), 0)?;
        shm_file.sync()?;
...
/// — [`crate::storage::row::header::DatabaseHeader::journal_mode`]'s same detection
...
/// `crate::storage::row::header::DatabaseHeader::journal_mode`'s detection bytes) with
...
mod tests {
    use super::*;
    use crate::storage::row::vfs::MemoryVfs;
    ...
    fn hot_journal_open_fails_when_another_connection_holds_reserved() {
        use crate::storage::row::vfs::test_lock_probe::lock_held_by_subprocess;
        use crate::storage::row::vfs::UnixVfs;
        ...
        let (start, len) = crate::storage::row::vfs::lock::reserved_byte_range();
        let result = lock_held_by_subprocess(&path, "wrlock", start, len, || {
            Pager::open(&UnixVfs, &path, page_size)
        });

        match result {
            Err(PagerError::Vfs(crate::storage::row::vfs::VfsError::Locked { .. })) => {}
    ...
    fn open_acquires_shared_lock_released_on_drop() {
        use crate::storage::row::vfs::lock::exclusive_lock_available;
        use crate::storage::row::vfs::UnixVfs;
        use std::sync::atomic::{AtomicU64, Ordering};
    ...
    fn open_claims_wal_read_lock_when_shm_present_released_on_drop() {
        use crate::storage::row::vfs::shm::slot_is_free_test_only;
        use crate::storage::row::vfs::UnixVfs;
        use std::sync::atomic::{AtomicU64, Ordering};
