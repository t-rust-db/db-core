...
mod tests {
    ...
    fn lock_shared_fails_with_contention_errno_when_exclusively_held_elsewhere() {
        use crate::storage::row::vfs::{UnixVfs, Vfs, VfsError};
    ...
    fn reserved_lock_available(path: &std::path::Path) -> bool {
        crate::storage::row::vfs::test_lock_probe::lock_available(path, "wrlock", RESERVED_BYTE, 1)
    }

    fn pending_lock_available(path: &std::path::Path) -> bool {
        crate::storage::row::vfs::test_lock_probe::lock_available(path, "wrlock", PENDING_BYTE, 1)
    }

    fn shared_read_available(path: &std::path::Path) -> bool {
        crate::storage::row::vfs::test_lock_probe::lock_available(
            path,
            "rdlock",
            SHARED_FIRST,
            SHARED_SIZE,
