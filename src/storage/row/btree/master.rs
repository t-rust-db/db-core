...
use crate::storage::row::record::{decode_record, encode_record, TextEncoding, Value};
...
/// (bytes 40-43) — see [`crate::storage::row::header::DatabaseHeader::schema_cookie`].
...
fn max_rowid(
    ...
    let mut cursor = crate::storage::row::btree::TableCursor::new(&*pager, header, root_page);
    let mut max = None;
...
fn find_master_rootpage(
    ...
    let mut cursor =
        crate::storage::row::btree::TableCursor::new(&*pager, header, SQLITE_MASTER_ROOT_PAGE);
    let mut row = cursor.first_row()?;
    while let Some(r) = row {
...
pub fn update_sequence(
    ...
    let mut cursor = crate::storage::row::btree::TableCursor::new(&*pager, header, seq_root);
...
pub fn delete_stat1_rows_for_table(
    ...
    let mut cursor = crate::storage::row::btree::TableCursor::new(&*pager, header, stat1_root);
...
mod tests {
    ...
    fn update_sequence_only_grows() {
        ...
        let mut cursor2 = crate::storage::row::btree::TableCursor::new(&pager, &header, seq_root);
