...
//! Lock contention on either surfaces as [`sql_vfs::VfsError::Locked`].
...
use sql_header::{JournalMode, SynchronousMode};
use sql_vfs::{
...
pub struct Pager {
    ...
    tx_lock_level: sql_vfs::LockLevel,
    ...
    /// see [`sql_vfs::Vfs::open_wal_shm`]'s doc comment. Lazily opened
...
/// Byte offsets of the three header fields ([`sql_header::DatabaseHeader`])
...
impl Pager {
    ...
    pub fn open<V: Vfs + Clone + 'static>(
        ...
        Ok(Pager {
            lock,
            tx_lock_level: sql_vfs::LockLevel::Shared,
            wal_lock,
            wal_shm: None,
            wal_resume: None,
            source,
    ...
    pub fn flush(&mut self) -> Result<(), PagerError> {
        ...
        self.lock.set_level(sql_vfs::LockLevel::Exclusive)?;
        ...
        self.lock.set_level(sql_vfs::LockLevel::Shared)?;
        self.tx_lock_level = sql_vfs::LockLevel::Shared;
        result
    }
    ...
    /// with [`VfsError::Locked`](sql_vfs::VfsError::Locked) if another
    /// connection already holds RESERVED or higher.
    pub fn begin_immediate(&mut self) -> Result<(), PagerError> {
        self.lock.set_level(sql_vfs::LockLevel::Reserved)?;
        self.tx_lock_level = sql_vfs::LockLevel::Reserved;
        Ok(())
    }
    ...
    pub fn begin_exclusive(&mut self) -> Result<(), PagerError> {
        self.lock.set_level(sql_vfs::LockLevel::Exclusive)?;
        self.tx_lock_level = sql_vfs::LockLevel::Exclusive;
        Ok(())
    }
    ...
    #[cfg(test)]
    pub fn tx_lock_level(&self) -> sql_vfs::LockLevel {
        self.tx_lock_level
    }
    ...
    fn release_tx_lock(&mut self) -> Result<(), PagerError> {
        if self.tx_lock_level > sql_vfs::LockLevel::Shared {
            self.lock.set_level(sql_vfs::LockLevel::Shared)?;
            self.tx_lock_level = sql_vfs::LockLevel::Shared;
        }
        Ok(())
    ...
    ///
    /// Claims [`sql_vfs::Vfs::claim_wal_write_lock`] for the duration,
    /// so a second concurrent writer is refused (surfaces as
    /// [`sql_vfs::VfsError::Locked`], converted to [`PagerError::Vfs`]
    ...
    pub fn set_journal_mode(&mut self, mode: JournalMode) -> Result<(), PagerError> {
        if !self.dirty.is_empty() || self.tx_lock_level > sql_vfs::LockLevel::Shared {
    ...
    fn recreate_wal_locked(&mut self) -> Result<wal::WalWriter, PagerError> {
        ...
        shm_file.write_at(&sql_vfs::shm::fresh_shm_bytes(), 0)?;
        shm_file.sync()?;
...
/// — [`sql_header::DatabaseHeader::journal_mode`]'s same detection
...
/// `sql_header::DatabaseHeader::journal_mode`'s detection bytes) with
...
///
/// A thin newtype around `RefCell<Pager>`, rather than implementing
/// [`PageSource`] on `RefCell<Pager>` directly: now that `PageSource`
/// (`sql-vfs`) and `Pager` (`sql-pager`) live in different crates, the
/// orphan rule forbids implementing a foreign trait on `RefCell<Pager>`
/// (`RefCell` is foreign too, and isn't a "fundamental" type the orphan
/// rule looks through) — `SharedPager` is local to this crate, so the
/// impl is allowed. `borrow`/`borrow_mut` forward to the same methods on
/// the inner `RefCell`, so callers use it exactly as they would the
/// `RefCell` itself.
pub struct SharedPager(std::cell::RefCell<Pager>);

impl SharedPager {
    /// Wraps `pager` for sharing between a read-only `PageSource` view
    /// and concrete `&mut Pager` write access.
    pub fn new(pager: Pager) -> Self {
        SharedPager(std::cell::RefCell::new(pager))
    }

    /// See [`std::cell::RefCell::borrow`].
    pub fn borrow(&self) -> std::cell::Ref<'_, Pager> {
        self.0.borrow()
    }

    /// See [`std::cell::RefCell::borrow_mut`].
    pub fn borrow_mut(&self) -> std::cell::RefMut<'_, Pager> {
        self.0.borrow_mut()
    }
}

impl PageSource for SharedPager {
    fn read_page(&self, page_num: u32) -> Result<Rc<[u8]>, PageError> {
...
mod tests {
    ...
    fn hot_journal_open_fails_when_another_connection_holds_reserved() {
        use sql_vfs::test_lock_probe::lock_held_by_subprocess;
        use sql_vfs::UnixVfs;
    ...
    fn open_acquires_shared_lock_released_on_drop() {
        use sql_vfs::lock::exclusive_lock_available;
        use sql_vfs::UnixVfs;
        use std::sync::atomic::{AtomicU64, Ordering};
    ...
    fn open_claims_wal_read_lock_when_shm_present_released_on_drop() {
        use sql_vfs::shm::slot_is_free_test_only;
        use sql_vfs::UnixVfs;
        use std::sync::atomic::{AtomicU64, Ordering};
