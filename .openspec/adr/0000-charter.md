...
| (f) | Oracle parity is the definition of correct: sqlite-rs's corpus and sqllogictest against the pinned sqlite3, on the pinned db-core. The column and stream modes have no sqlite3-shaped counterpart, but each has its own oracle (db-core#406): column checks the engine's query results against DuckDB reading the same Parquet fixtures; stream's tests independently re-derive the expected answer from the raw fixture bytes ("oracle by construction") instead of trusting the engine's own parser. | sqlite-rs CI (`corpus`), sqlite-rs `assurance.yml` (weekly); column: `make check-column-oracle`, local-only, run by hand, never CI (a DuckDB binary is a new external dependency this charter does not accept onto the machine that runs untrusted PRs); stream: `tests/unit/engine_stream_public_api_test.rs`'s `oracle()`/`json_status_oracle()` helpers, run as ordinary tests | every sqlite-rs PR; weekly; column: manual; stream: every PR |
| (g) | One 85% line-coverage floor over all of db-core (`--all-features`), checked crate-wide and per-file -- a crate-wide-only number lets well-covered files mask thin ones. No blanket exemption for any module; an exception must be named individually here with a justification (db-core#407: there are none). | `make check-coverage` | every PR |
